...
public class DefaultMessagesService implements MessagesService {
    ...
    public List<String> getAllMessages() {
        ...
        if (messages == null) {
            ...
            for (Entity entity : query.asIterable(
                    FetchOptions.Builder.withLimit(Constants.MAX_COUNT))) {
    ...
    public void receiveMessage(PubsubMessage pubsubMessage) throws IOException {
        ...
        System.out.println("rawMessage = " + pubsubMessage.toPrettyString());

        Map<String,Object> map = (Map)pubsubMessage.get("message");

        map.keySet().stream().forEach(k ->System.out.println("key,val =" + k + " " + map.get(k)));

        String encodedData = (String)map.get("data");
        System.out.println("dencodedData = " + encodedData);
        byte[] rawData = Base64.decodeBase64(encodedData);
        String decodedData = new String(rawData,"UTF-8");
        System.out.println("byte[] decoded from pubsubMessage = " + pubsubMessage.decodeData());
        System.out.println("received message as " + decodedData);
    }
